...
def create_app_icon():
    """Create a simple in-memory icon so the desktop app does not depend on asset files."""
...
def get_next_filename(base_name="synthetic_dataset", extension=".jsonl"):
    """Return a non-clobbering output filename by suffixing an incrementing counter."""
    output_dir = Path(".")
...
def cosine_similarity(v1, v2):
    """Safely compute cosine similarity even when an embedding contains only zeros."""
...
def extract_json_from_response(content):
    ...
    Models often wrap responses in markdown fences or surround JSON with extra prose.
    This helper keeps the parser tolerant while still preferring the first balanced
    object over a greedy regex match.
    """
...
def parse_json_object(content, progress_signal=None, context="response"):
    """Parse a JSON object from model output and raise a clear error on failure."""
...
def normalize_topic(candidate):
    """Normalize topic text so duplicate detection is resilient to spacing/casing noise."""
...
class TopicRelevanceValidator:
    ...
    def is_relevant(self, main_topic, sub_topic_candidate):
        try:
            ...
            data = parse_json_object(response["message"]["content"], self.progress_signal, "relevance validator response")
...
class TextSnippetValidator:
    ...
    def validate_relevance(self, topic, generated_text):
        ...
        try:
            ...
            data = parse_json_object(response["message"]["content"], self.progress_signal, "snippet validator response")
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                while len(accepted_topics) < self.target_topic_count and attempts < max_attempts:
                    ...
                    data = parse_json_object(response["message"]["content"], self.progress, "topic generator response")
                    candidate_topics = data.get("topics", [])
                    if not candidate_topics:
                        self.progress.emit(("   - LLM returned an empty topic list for the batch. Retrying.", "WARN"))
                        continue
                    ...
                    for raw_candidate in candidate_topics:
                        ...
                        if not isinstance(raw_candidate, str):
                            self.progress.emit((f"   - REJECTED (Invalid Type): {raw_candidate}", "WARN"))
                            continue

                        candidate = re.sub(r"\s+", " ", raw_candidate.strip())
                        normalized_candidate = normalize_topic(candidate)
                        if not candidate:
                            self.progress.emit(("   - REJECTED (Empty Topic)", "WARN"))
                            continue
                        if normalized_candidate in seen_topics:
                            self.progress.emit((f"   - REJECTED (Duplicate): '{candidate}'", "WARN"))
                            continue
                        if not self.relevance_validator.is_relevant(self.main_topic, candidate):
                            self.progress.emit((f"   - REJECTED (Irrelevant): '{candidate}'", "WARN"))
                            continue
                        ...
                        is_too_similar = any(
                            cosine_similarity(candidate_embedding, embedding) > self.similarity_threshold
                            for embedding in accepted_embeddings
                        )
                        if is_too_similar:
                            self.progress.emit((f"   - REJECTED (Similarity): '{candidate}'", "WARN"))
                            continue
                ...
                if len(accepted_topics) < self.target_topic_count:
                    self.error.emit(f"Topic generation stopped after producing {len(accepted_topics)} of {self.target_topic_count} requested topics.")
                    return
...
class DataGenerationWorker(QThread):
    ...
    def run(self):
        ...
        if num_sub_topics == 0:
            self.error.emit("Cannot generate data with an empty list of sub-topics.")
            return

        attempts = 0
        max_attempts = self.total_samples * 3

        while len(dataset) < self.total_samples and attempts < max_attempts:
            sample_index = len(dataset)
            current_topic = self.sub_topics[sample_index % num_sub_topics]
            ...
            try:
                ...
                text_entry = parse_json_object(response["message"]["content"], self.progress, "data generation response")
                generated_text = text_entry.get("text", "").strip()
                if len(generated_text) < MIN_SNIPPET_LENGTH:
                    self.progress.emit(("   - Generation failed: Text too short. Retrying.", "WARN"))
                    attempts += 1
                    continue

                is_relevant, reason = self.validator.validate_relevance(current_topic, generated_text)
                if not is_relevant:
                    self.progress.emit((f"   - Snippet relevance FAILED. Reason: {reason}. Discarding.", "WARN"))
                    attempts += 1
                    continue
                ...
                self.progress.emit((f"   - Failed to generate or parse snippet: {exc}. Retrying.", "WARN"))
                attempts += 1
                time.sleep(2)

        if len(dataset) < self.total_samples:
            self.error.emit(f"Failed to generate {self.total_samples} samples after {attempts} attempts.")
            return
...
class SyntheticDatasetGenerator(QWidget):
    ...
    def populate_model_combos(self):
        """Prefer current Qwen models, but merge in locally installed Ollama models for flexibility."""
        ...
        try:
            ...
            # The app should still launch even when Ollama is not reachable yet.
            installed_model_names = []
